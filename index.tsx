...
function getListingQualityScore(item: Listing) {
  let score = 20;

  if (item.status === "Verified") score += 15;
  if (item.ownerVerified) score += 18;
  if (item.ownerName) score += 8;
  if (item.ownerPhone || item.whatsapp) score += 8;
  if (item.documentTitle || item.documentName) score += 14;
  if (item.imageUrl) score += 8;
  if (item.galleryUrls && item.galleryUrls.length > 0) score += 6;
  if (item.landSize) score += 5;
  if (item.summary && item.summary.length > 70) score += 8;
...
export default function App() {
  ...
  function approveAllPendingListings() {
    const pendingCount = listings.filter((item) => item.status !== "Verified").length;
    ...
    setListings((currentListings) =>
      currentListings.map((item) => ({ ...item, status: "Verified" }))
    );

    showMessage(`${pendingCount} pending listing${pendingCount === 1 ? "" : "s"} approved.`);
  }

  function verifyAllOwners() {
    const unverifiedCount = listings.filter((item) => !item.ownerVerified).length;
    ...
    showMessage(`${unverifiedCount} owner / agent profile${unverifiedCount === 1 ? "" : "s"} verified.`);
  }

  function featureTopViewedListings() {
    ...
    if (!topIds.length) {
      showMessage("Open a few property details first so top-viewed listings can be detected.");
      return;
    }
    ...
    setListings((currentListings) =>
      currentListings.map((item) => ({
        ...item,
        featured: topIds.includes(item.id),
      }))
    );

    showMessage("Top viewed listings are now featured.");
  }
  ...
  function clearBuyerActivity() {
    ...
    showMessage("Recent views, compare list, and saved notes cleared.");
  }
        ...
        <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-[2rem] border border-emerald-100 bg-emerald-50 p-5 lg:p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-emerald-800">
              These numbers are calculated from current listings in this browser so the marketplace feels more professional for buyers and investors.
            </p>

            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-white p-4">
                <p className="text-2xl font-black text-[#0d1c38]">
                  {formatNairaAmount(totalMarketValue)}
                </p>
                <p className="mt-1 text-xs font-black uppercase tracking-wide text-slate-400">
                  Listed value
                </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white p-4">
                  ...
                  <p className="mt-1 text-xl font-black text-[#0d1c38]">
                    {formatNairaAmount(calculatorEstimatedReturn)}
                  </p>
                </div>
                ...
                <div className="rounded-2xl bg-white p-4">
                  ...
                  <p className="mt-1 text-xl font-black text-[#0d1c38]">
                    {formatNairaAmount(calculatorProjectedTotal)}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="rounded-2xl border border-[#f0bf3c]/30 bg-[#fff7df] p-4">
                  <div className="mb-4">
                    ...
                    <p className="mt-1 text-xs leading-5 text-[#9b6b16]">
                      Fast professional controls for approving, verifying, featuring, and clearing buyer activity.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      type="button"
                      onClick={featureTopViewedListings}
